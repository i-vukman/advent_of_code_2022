...
impl FileSystemNode {
    ...
    pub fn sum_folders_with_size_less_than(&self, size_limit: u32) -> u32 {
        ...
        let folder_size = if folder_size <= size_limit {
            folder_size
        } else {
            0
        };
    ...
    pub fn find_smallest_folder_size_with_size_larger_than(
        &self,
        minimum_size: u32,
    ) -> Option<u32> {
    ...
    fn add_node_to_relative_path(&mut self, relative_path: &str, size: u32) {
        ...
        if is_node_name {
            ...
            self.children
                .push(FileSystemNode::new(node_name.to_string(), size));
            return;
        }
        ...
        let folder_name = relative_path
            .split('/')
            .next()
            .expect("Iterator_can't be empty");
        let new_relative_path = &relative_path[folder_name.len() + 1..];

        let existing_folder = self
            .children
            .iter_mut()
            .find(|child| child.is_folder() && child.name == folder_name);
...
pub fn solve_02(input: &str) -> u32 {
    ...
    root.find_smallest_folder_size_with_size_larger_than(total_diff)
        .unwrap()
}
...
fn parse_file_system_tree(input: &str) -> FileSystemNode {
    ...
    input.lines().for_each(|line| match line {
        _l if line.starts_with("$ cd") => {
            ...
            match path {
                ".." => {
                    while current_path
                        .pop()
                        .expect(&format!("{current_path} should not be empty"))
                        != '/'
                    {}
                }
        ...
        _ => {
            ...
            root.add_file_to_relative_path(
                relative_path,
                size.parse().expect(&format!("Invalid file size {size}")),
            );
        }
    });
...
mod tests {
    ...
    fn test_part_01() {
        ...
        assert_eq!(super::solve_01(&input), 1648397);
    }

    #[test]
    fn test_sample() {
        let input = fs::read_to_string("input/day_07_sample.txt").unwrap_or(String::from(""));
        assert_eq!(super::solve_01(&input), 95437);
    }

    #[test]
    fn test_part_02() {
        let input = fs::read_to_string("input/day_07.txt").unwrap_or(String::from(""));
        assert_eq!(super::solve_02(&input), 1815525);
    }
}
